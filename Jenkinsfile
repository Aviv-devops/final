    ...
    stages {
        ...
        */
        ...
        stage("Connect To ECR") {
            steps{
                sshagent(credentials:['devops']) {
                    ...
                    //sh 'ssh -t -o StrictHostKeyChecking=yes ubuntu@34.229.242.33 "echo heyheyhey"'
                    sh 'ssh -T -o StrictHostKeyChecking=no ubuntu@204.236.193.84 "docker login -u AWS -p $(aws ecr get-login-password --region us-east-1) 808447716657.dkr.ecr.us-east-1.amazonaws.com"'
        ...
        stage("pull image") {
            steps{
                sshagent(credentials:['devops']) {
                    ...
                    sh 'ssh -T ubuntu@204.236.193.84 "docker pull 808447716657.dkr.ecr.us-east-1.amazonaws.com/final_project:$BUILD_ID"'
                }
            }
        }
        ...
        stage("Docker run") {
            steps{
                sshagent(credentials:['devops']) {
                    sh 'ssh -T ubuntu@204.236.193.84 "docker run -p 8000:8000 --name yarden$BUILD_ID -td 808447716657.dkr.ecr.us-east-1.amazonaws.com/final_project:$BUILD_ID"'
                }
            }
        }
